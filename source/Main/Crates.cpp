...
bool Crates::updateMain(){
    if(breakAnim){
        ...
        Position.z = 0;
        if(settings>>24 & 0xf != 0){
            Actor::spawnActor(31, (settings>>12 & 0xf)*0x111100000 + ((settings>>8 & 0xf)),  &this->position);
        }
        for (int i = 0; i < (settings>>0 & 0xf) + 1; i++) {
            Actor::spawnActor(66, 0x4,  &this->position);
        }
        for (int i = 0; i < (settings>>4 & 0xf) + 1; i+=2) {
            Actor::spawnActor(66, 0x4,  &this->position);
        }
        this->Base::destroy();
